...
public class CommonFunctions {
    ...
    public static void LogPlayerAction(EntityLivingBase living, String action){
        ActFramework.Log(String.format("%s(%s): %s",living.getName(), living.getUniqueID(), action));
    }
        ...
        while (costLeft > 0 && (player.experienceLevel > 0 || player.experience > 0))
        {
            if (player.experience > costLeft)
            {
                ...
                if (player.experienceLevel > 0)
                {
                    ...
                    ActFramework.Log(String.format("player.experience = %d", XPForLevel(player.experienceLevel)));
                }
            }
        }
        ActFramework.Log(String.format("Lv= %s, xp = %s", player.experienceLevel, player.experience));
        return true;
    }
