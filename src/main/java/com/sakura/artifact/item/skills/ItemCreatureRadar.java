package com.sakura.artifact.item.skills;

import com.sakura.artifact.util.CommonFunctions;
import com.sakura.artifact.util.IDLGeneral;
        ...
        if (!world.isRemote) {
            ...
            if (!cloneResult.isEmpty())
            {
                ...
                if (name != null && EntityList.ENTITY_EGGS.containsKey(name))
                {
                    ...
                    ActFramework.LogWarning(IDLNBTUtil.getNBT(stack).toString());
...
//        Entity entity = EntityList.createEntityByIDFromName(resourcelocation, worldIn);
    ...
    public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        ...
        if (!worldIn.isRemote)
        {
            ...
            ActFramework.Log("update:", IDLNBTUtil.getNBT(stack).toString());
            ...
            {
                //ActFramework.Log(String.format("[Active]Nearby %s -> %s" , entity.getName() ,entity.getAttackTarget()));
                if (entity.getClass() == s && entity != entityIn)
                {
                    detection++;
                    //ActFramework.Log("[Active]Detected!");
                }
            }
            ...
            {
                ActFramework.LogWarning("Changed to " + detection);
                IDLNBTUtil.SetInt(stack, IDLNBTDef.STATE, detection);
