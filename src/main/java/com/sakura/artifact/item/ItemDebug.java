package com.sakura.artifact.item;

import com.sakura.artifact.entity.projectiles.EntityIdlProjectile;
import com.sakura.artifact.entity.projectiles.ProjectileArgs;
...
import static com.sakura.artifact.util.IDLSkillNBT.GetGuaEnhance;
...
//    }
    ...
    void TestEnchant(World worldIn, EntityPlayer playerIn, EnumHand handIn)
    {
        ...
        for (Enchantment enchantment : Enchantment.REGISTRY)
        {
            ...
            ActFramework.Log("[%s]Lv %d to %d, canApplyAtEnchantingTable = %s, judgement[%s,%s,%s]",
                    enchantment.getName(),
            ...
            if (judge1 && judge2 || judge3)
            {
                ...
                {
                    ActFramework.Log("[%s] Lv.%d available range = %d to %d", enchantment.getName(), i,
                    ...
                    {
                        ActFramework.Log("[%s] successfully elected", enchantment.getName());
                        break;
                    }
                }
            }
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        if (index == 2 && !worldIn.isRemote) {
            ...
            ActFramework.Log("bullet pos = %s", bullet.getPosition());
        ...
        if (!worldIn.isRemote)
        {
            ...
            if (index == 3)
            {
                ...
                playerIn.openGui(ActFramework.instance, ModGuiElementLoader.GUI_RESEARCH, worldIn, pos.getX(), pos.getY(), pos.getZ());
            }
            //else

            //todo: optimize
            //playerIn.openGui(ActFramework.instance, ModGuiElementLoader.GUI_DEMO, worldIn, pos.getX(), pos.getY(), pos.getZ());
...
//            }
        ...
        //ActFramework.Log("NBT len = %d", IDLNBTUtil.GetString(stack, "TEST", "").length());
